...
Your puzzle answer was `20626`.

**Both parts of this puzzle are complete! They provide two gold stars: \*\***

## Solution for part one
Part one is realtively easy to brute force, it just takes a while to complete. An observation is since each button press **toggles** the corresponding light, each button can only be pressed once. Pressing it twice just removes the effect of pressing it in the first place. Pressing it thrice is equivalent to pressing it once. So we can iterate through the buttons pressing each once, then for each press trying one of the remaining buttons and so on. The lighths can be represented by a single integer with each bit representing a light. This allows us to consider the buttons as integers as well and using XOR to flip the lights (bits). This works but takes a while to complete. Add memoization and it runs i the ms range instead of seconds.  My biggest grief with todays puzzle is parsing the input... it made for an ugly function!
```
BenchmarkTools.Trial: 27 samples with 1 evaluation per sample.
 Range (min … max):  182.868 ms … 197.082 ms  ┊ GC (min … max): 3.95% … 9.70%
 Time  (median):     188.732 ms               ┊ GC (median):    7.58%
 Time  (mean ± σ):   188.583 ms ±   3.317 ms  ┊ GC (mean ± σ):  7.64% ± 1.13%

                  ▃          █     ▃                             
  ▇▇▁▁▇▁▁▁▁▁▇▇▇▇▇▁█▁▁▇▁▇▁▁▇▇▁█▇▁▇▁▁█▁▁▇▁▇▁▇▇▁▁▁▇▁▁▁▁▁▁▁▁▁▁▁▁▁▁▇ ▁
  183 ms           Histogram: frequency by time          197 ms <

 Memory estimate: 327.20 MiB, allocs estimate: 5440989.
 ```

## Solution for part two
In part two we switch the lighing configurations for target joltage levels. And instead of flipping bits we add 1 to the respetive positions in the joltage meter for each button press. Brute forcing this would take forever. But since we're adding numbers in a nice linear fashion this equates to a liner system of equations where we have the coefficents (the counters each button adds to) and the result (the target joltage levels). Note that there can be several answers to each system with the same number of button presses. But since we only need to look for the total number of presses this doesn't really matter. We can now compute the number of presses by using x = A\\b where A is the coefficent matrix, b the target joltage levels and x a vector contaiing the number of presses for each button. Julia's built in solver gives real answers, not integer ones. Substitution with a varible number of unknowns is a bother. Gaussian elimination would work, but takes a while to implement. Luckily Julia has the JuMP library that can be used to solve linear equations with integer answers. Learning how to use it took a while. But it works like a charm.
```
BenchmarkTools.Trial: 17 samples with 1 evaluation per sample.
 Range (min … max):  295.155 ms … 324.974 ms  ┊ GC (min … max): 0.00% … 1.75%
 Time  (median):     302.533 ms               ┊ GC (median):    0.75%
 Time  (mean ± σ):   303.583 ms ±   6.827 ms  ┊ GC (mean ± σ):  0.63% ± 0.48%

  ▁ ▁ ▁▁ ▁     ▁█▁ █▁   ▁▁   █                                ▁  
  █▁█▁██▁█▁▁▁▁▁███▁██▁▁▁██▁▁▁█▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁█ ▁
  295 ms           Histogram: frequency by time          325 ms <

 Memory estimate: 40.11 MiB, allocs estimate: 1232014.
 ```