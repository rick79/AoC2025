...
Today is a case of solving the problem, not the assignment. Stacking a large number of tetrominoes in large areas is a rather hard problem so solve. If we look at this as a 2D packing problem there are several strategies we can employ: rules based strategies, heuristics based strategies, and combinations of rules based and heuristics based strategies. An not to forget, AI based strategies. But lets begin with looking at the data. Just glancing through the puzzle input (not the test data given in the problem) there are a lot of shapes that are supposed to be stacked in each area. Too many. All too many. So let us begin by counting how many areas that can fit all shapes if we just place them side by side. And then let us look at the remaining areas just to get a picture of how big the difference is between the number of shapes we can fit by placing them side by side and the required number of shapes. Turns out there are about 3 times more required shapes than there are available space when placing side by side. And stacking the shapes so that they take one third, or even one half, the size is just not plausible. So let us just try to enter the number of regions that can fit all it's required shapes when placing the shapes side by side without stacking. Lo and behold, it works!